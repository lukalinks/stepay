...
export async function POST(request: Request) {
    try {
        ...
        if (await hasServerStoredSecret(userId)) {
            ...
            const payload = parseSendPayload(coerceIntentPayload(intent.payload));
            try {
        ...
        if (row.status === 'confirmed') {
            ...
            if (!plan) {
                return NextResponse.json({ error: 'Missing transaction plan. Please start send again.' }, { status: 400 });
            }
            return NextResponse.json({
                success: true,
