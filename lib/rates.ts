...
let cacheExpiry = 0;
// Short TTL so admin changes propagate across Vercel serverless instances (each has its own cache)
const CACHE_MS = 2_000; // 2 sec

async function fetchFromDb<T>(key: string, defaultVal: T): Promise<T> {
  try {
    const rows = await sql`
      SELECT value_json FROM platform_settings WHERE key = ${key} LIMIT 1
    `;
...
/** Quote crypto-to-crypto swap using platform sell/buy rates (spread included). */
export function swapQuote(
  ...
  rates: PlatformRates,
  fees: PlatformFees
): { toAmount: number; zmwValue: number } {
  if (from === to) return { toAmount: amount, zmwValue: 0 };
  const sellMult = 1 - fees.sell_percent / 100;
  const buyMult = 1 - fees.buy_percent / 100;
  const zmwValue =
    from === 'xlm' ? amount * rates.xlm_sell * sellMult : amount * rates.usdc_sell * sellMult;
  const toAmount =
    to === 'usdc' ? (zmwValue / rates.usdc_buy) * buyMult : (zmwValue / rates.xlm_buy) * buyMult;
  return { toAmount, zmwValue };
}
