...
export class StellarService {
    ...
    /**
     * Ensure account has enough XLM to add a USDC trustline (top up from platform if needed).
     */
    static async ensureXlmForUsdcTrustline(
        platformSecret: string,
        walletPublic: string
    ): Promise<void> {
        const exists = await this.accountExists(walletPublic);
        if (!exists) {
            await this.sendXLM(platformSecret, walletPublic, String(MIN_XLM_NEW_ACCOUNT_FOR_USDC));
            return;
        }
        const balance = Number(await this.getBalance(walletPublic));
        if (balance < MIN_XLM_FOR_USDC_TRUSTLINE) {
            const topUp = (MIN_XLM_FOR_USDC_TRUSTLINE - balance + 0.05).toFixed(7);
            await this.sendXLM(platformSecret, walletPublic, topUp);
        }
    }
